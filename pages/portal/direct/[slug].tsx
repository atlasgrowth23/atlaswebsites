...
export default function DirectPortal({ companyName, slug, companyData }: Props) {
...
export const getServerSideProps: GetServerSideProps = async ({ req, params, query }) => {
  ...
  try {
    // Track this portal view for analytics
    await directAuth.trackPortalView(
      slug, 
    ...
    // Get company data
    const company = await directAuth.getCompanyData(slug);
    if (!company) {
      console.log(`No company found for slug: ${slug}`);
      return { notFound: true };
    }
    
    console.log(`Direct portal access for ${company.name} (${slug})`);
    ...
    console.error('Error loading direct portal:', error);
    return { notFound: true };
  }
};