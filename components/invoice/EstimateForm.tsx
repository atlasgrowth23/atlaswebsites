...
import { Estimate, EstimateItem, EstimateStatusLabels, StandardTermsTemplate } from '@/types/invoice';
...
export default function EstimateForm({
    ...
    async function fetchInvoiceSettings() {
      try {
        ...
        if (data.success && data.settings) {
          ...
          // If it's a new estimate, apply default settings
          if (!isEdit && !estimateData?.id) {
            setFormData(prev => ({
              ...prev,
              estimate_number: data.settings.next_estimate_number ? `EST-${data.settings.next_estimate_number}` : prev.estimate_number,
              terms: data.settings.estimate_terms_template || prev.terms
            }));
          }
        }
      } catch (apiError) {
  ...
  }, [businessId, isEdit, estimateData]);
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                  <Label htmlFor="estimate_number">Estimate Number <span className="text-red-500">*</span></Label>
                  <Input
                    id="estimate_number"
                    ...
                    required
                    className="mt-1"
                    disabled={isEdit} // Don't allow changing estimate number on edit
                  />
                </div>
                ...
                <div>
                  <Label htmlFor="date_expires">Expiration Date <span className="text-red-500">*</span></Label>
                  <Input
                    id="date_expires"
                    name="date_expires"
                    type="date"
                    value={formatDateForInput(formData.date_expires)}
                    onChange={handleInputChange}
                    required
                    className="mt-1"
                  />
              ...
              <div className="grid grid-cols-1 gap-4">
                <div>
                  ...
                  <Textarea
                    ...
                    rows={4}
                    className="mt-1"
                    placeholder="Enter any notes for this estimate"
                  />
                </div>
