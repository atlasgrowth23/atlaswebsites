...
const Footer: React.FC<FooterProps> = ({ company }) => {
  ...
  return (
    <footer id="contact" className="bg-gradient-to-r from-blue-900 to-blue-700 text-white py-12">
      ...
      {/* Custom script to replace widget avatar with business logo */}
      {company.logoUrl && (
        <script
            ...
            __html: `
              (function() {
                function customizeWidget() {
                  ...
                  setTimeout(function() {
                    try {
                      // Find widget avatar/image elements (common selectors for GoHighLevel widgets)
                      ...
                      // Try to find and replace avatar image
                      avatarSelectors.forEach(selector => {
                        const elements = document.querySelectorAll(selector);
                        elements.forEach(element => {
                          if (element.tagName === 'IFRAME') {
                            // For iframe widgets, we need to access the iframe content
                            try {
                              const iframeDoc = element.contentDocument || element.contentWindow.document;
                              const iframeAvatar = iframeDoc.querySelector('img');
                              if (iframeAvatar) {
                                iframeAvatar.src = '${company.logoUrl}';
                                iframeAvatar.style.borderRadius = '50%';
                                iframeAvatar.style.objectFit = 'contain';
                                iframeAvatar.style.backgroundColor = 'white';
                                iframeAvatar.style.padding = '4px';
                                avatarFound = true;
                              }
                            } catch (e) {
                      ...
                      // If no avatar found, try again in a bit
                      if (!avatarFound) {
                        setTimeout(customizeWidget, 2000);
                      }
                    } catch (error) {
                      console.log('Widget customization note:', error.message);
                    }
                  }, 1000);
                }
                
                // Run customization when page loads and periodically
                if (document.readyState === 'loading') {
                  document.addEventListener('DOMContentLoaded', customizeWidget);
                } else {
                  customizeWidget();
                }
                
                // Also try after a delay in case widget loads later
                setTimeout(customizeWidget, 3000);
                setTimeout(customizeWidget, 5000);
              })();
            `
          }}
        />
