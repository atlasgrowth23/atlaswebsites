...
const GoogleReviews: React.FC<GoogleReviewsProps> = ({ company }) => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        ...
        // Filter 5-star reviews for this company by place_id
        ...
        setReviews(companyReviews.slice(0, 10)); // Show up to 10 reviews
      } catch (error) {
  ...
  if (reviews.length < 3) {
    return null; // Don't show if less than 3 reviews
  }
