...
async function importGoogleReviews() {
  try {
    ...
    // Start the streaming approach
    const parser = parse({
      columns: true,
      skip_empty_lines: true,
      trim: true,
      relax_column_count: true, // Handle uneven column counts
      relax_quotes: true, // Handle relaxed quote parsing
    ...
    // Create and execute stream pipeline
    ...
    try {
      ...
      parser.on('end', async function() {
        try {
          ...
          console.error('Final transaction error:', error.message);
        } finally {
          client.release();
        }
      });
      ...
      const input = fs.createReadStream(csvFilePath, { encoding: 'utf-8' });
      input.pipe(parser);
...
// Add run function to start the import
async function run() {
  try {
