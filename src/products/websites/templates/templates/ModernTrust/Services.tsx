...
const Services: React.FC<ServicesProps> = ({ company }) => {
  ...
  // Determine colors based on active type - consistent reds and blues
  const activeColors = {
    bg: activeType === 'cooling' ? 'bg-blue-600' : 'bg-red-700',
    text: activeType === 'cooling' ? 'text-blue-600' : 'text-red-700',
    bgLight: activeType === 'cooling' ? 'bg-blue-100' : 'bg-red-100',
    hover: activeType === 'cooling' ? 'hover:bg-blue-700' : 'hover:bg-red-800',
    border: activeType === 'cooling' ? 'border-blue-200' : 'border-red-200'
  };
